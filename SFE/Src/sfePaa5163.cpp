...
// PAA5163 to configure it. Here's that list!
const uint8_t performanceSettings[][2] = {
    {0x00, 0x00}, {0x3A, 0x5A}, {0x7F, 0x00}, {0x40, 0x80}, {0x7F, 0x14}, {0x4D, 0x00}, {0x53, 0x0D}, {0x4B, 0x10},
    ...
    {0x7F, 0x00}, {0x64, 0x7C}, {0x65, 0x0C}, {0x72, 0x0E}, {0x73, 0x00}, {0x7F, 0x14}, {0x61, 0x3E}, {0x62, 0x1E},
    ...
    {0x5D, 0x04}, {0x5E, 0xEC}, {0x7F, 0x05}, {0x42, 0x48}, {0x43, 0xE4}, {0x7F, 0x06}, {0x71, 0x03}, {0x7F, 0x09},
    ...
    {0x52, 0x04}, {0x7F, 0x0C}, {0x5B, 0x00}, {0x5E, 0x17}, {0x61, 0x2C}, {0x7F, 0x0D}, {0x71, 0x92}, {0x70, 0x07}, {0x73, 0x92},
    ...
    {0x6D, 0x96}, {0x6E, 0x2F}, {0x55, 0x62}, {0x59, 0x21}, {0x5F, 0xD8}, {0x6A, 0x22}, {0x7F, 0x07}, {0x42, 0x30}, {0x43, 0x00},
    {0x7F, 0x06}, {0x43, 0x96}, {0x44, 0x2C}, {0x4C, 0x01}, {0x54, 0x02}, {0x62, 0x01}, {0x7F, 0x09}, {0x41, 0x01},
    {0x4F, 0x00}, {0x55, 0xF0}, {0x56, 0xF0}, {0x7F, 0x0A}, {0x4C, 0x18}, {0x51, 0x8F}, {0x55, 0x4D}, {0x7F, 0x07},
    {0x40, 0x40}, {0x7F, 0x00}, {0x40, 0x80}, {0x7F, 0x09}, {0x40, 0x03}, {0x44, 0x08}, {0x4F, 0x08}, {0x7F, 0x0A},
    {0x51, 0x8E}, {0x7F, 0x08}, {0x4A, 0x04}, {0x7F, 0x00}, {0x66, 0x11}, {0x67, 0x08}
};
...
sfeTkError_t sfePaa5163::loadPerformanceSettings()
{
    // This follows the datasheet's procedure for loading the performance
    // optimization settings into the PAA5163. It mostly involves writing
...
sfeTkError_t sfePaa5163::begin(sfeTkStmSPI *spiBus)
{
...
sfeTkError_t sfePaa5163::isConnected()
{
    // Read the product ID
    uint8_t productID = 0;
    ...
    if (productID != kPaa5163ProductId)
        return kSTkErrFail;
    ...
    if (productID != (uint8_t)(~kPaa5163ProductId))
        return kSTkErrFail;

    return kSTkErrOk;
}

sfeTkError_t sfePaa5163::softReset()
{
    // Send soft reset command
    sfeTkError_t err = _spi->writeRegisterByte(0x3A, kPaa5163CmdSoftReset);
    if(err != kSTkErrOk)
        return err;
...
sfeTkError_t sfePaa5163::shutdown()
{
    return _spi->writeRegisterByte(0x3B, kPaa5163CmdShutdown);
}

sfeTkError_t sfePaa5163::getRevisionId(uint8_t &revisionId)
{
    return _spi->readRegisterByte(0x01, revisionId);
}

sfeTkError_t sfePaa5163::setResolution(uint16_t resX, uint16_t resY)
{
    // Ensure provided resolutions are valid
    if (resX > kPaa5163MaxResolution)
        return kSTkErrFail;
    if (resY > kPaa5163MaxResolution)
        return kSTkErrFail;
...
sfeTkError_t sfePaa5163::setResolution(uint16_t resXY)
{
    return setResolution(resXY, resXY);
}

sfeTkError_t sfePaa5163::getResolution(uint16_t &resX, uint16_t &resY)
{
    sfeTkError_t err = kSTkErrOk;
...
float sfePaa5163::rawToInchesX()
{
    return _rawToInchX;
}

float sfePaa5163::rawToMetersX()
{
    return _rawToMeterX;
}

float sfePaa5163::rawToInchesY()
{
    return _rawToInchY;
}

float sfePaa5163::rawToMetersY()
{
    return _rawToMeterY;
}

sfeTkError_t sfePaa5163::setOrientation(sfe_paa5163_orientation_t oritentation)
{
    sfe_paa5163_orientation_t reg;
...
sfeTkError_t sfePaa5163::getOrientation(sfe_paa5163_orientation_t &oritentation)
{
    return _spi->readRegisterByte(0x5B, oritentation.value);
}

sfeTkError_t sfePaa5163::readBurstData(uint8_t *dataBuffer, uint8_t bufferSize)
{
