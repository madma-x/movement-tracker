...
typedef union {
    ...
    uint8_t value;
} sfe_paa5163_orientation_t;

// Product ID
const uint8_t kPaa5163ProductId = 0x4F;

// Soft reset command
const uint8_t kPaa5163CmdSoftReset = 0x5A;

// Shutdown command
const uint8_t kPaa5163CmdShutdown = 0xB6;

// Resolution is 100 dpi per LSB
const uint8_t kPaa5163ResolutionPerLsb = 100;

// Max resolution is 20k dpi, but the register value is (resolution / 100) - 1
// so the max raw value is (20000 / 100) - 1 = 199
const uint16_t kPaa5163MaxResolution = 199;

class sfePaa5163
{
  public:
    /// @brief Begins the PAA5163
    /// @return 0 if successful, negative for errors, positive for warnings
    sfeTkError_t begin(sfeTkStmSPI *spiBus = nullptr);

    /// @brief Checks if the PAA5163 is connected
    /// @return 0 if successful, negative for errors, positive for warnings
    ...
    /// @brief Shuts down the PAA5163, must call softReset() to power up again
    /// @return 0 if successful, negative for errors, positive for warnings
    sfeTkError_t shutdown();

    /// @brief Gets the revision ID of the PAA5163
    ...
    /// @brief Sets the orientation values for the PAA5163, which can invert the
    /// X and Y axes, and swap the X and Y axes
    /// @param oritentation Orientation value
    /// @return 0 if successful, negative for errors, positive for warnings
    sfeTkError_t setOrientation(sfe_paa5163_orientation_t oritentation);

    /// @brief Gets the orientation values for the PAA5163, which can invert the
    /// X and Y axes, and swap the X and Y axes
    /// @param oritentation Orientation value
    /// @return 0 if successful, negative for errors, positive for warnings
    sfeTkError_t getOrientation(sfe_paa5163_orientation_t &oritentation);
  ...
  private:
    ...
    // Helper function to initialize the PAA5163
    sfeTkError_t init();
};